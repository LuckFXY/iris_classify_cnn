...
#params,模型的参数
    ...
    def getConfusion_matrix(self):
        return confusion_matrix(y,self.y_pred)
...
def ReadImg(folder_name,START_POS=[0,],reset=False):
    ...
    print 'filenum=',filenum[0],' start_pos=',start_pos
    ...
    for i in range(num):
        #print 'read: %d / %d \r' %(i+start_pos,filenum[0]),
...
def evaluate_lenet(datasets,params,skip_training,apply_set_namelist,
                    n_epochs=2,learning_rate=0.0003,batch_size=60,
                    nkerns=[20,40,60],sizekerns=[5,4,3]):#500
    '''
    ...
    if skip_training==False:
        batch_size=min(n_train_batches,n_valid_batches,batch_size)
        print (' train_size=%d\n valid_size=%d\n batch_size=%d')%\
            (n_train_batches,n_valid_batches,batch_size)
        n_train_batches /= batch_size  
        n_valid_batches /= batch_size  #需要修改if data_size<batch_size batch=size
    ...
    #我们加载数据是(batch_size,220*220),但是LeNetConvPoolLayer输入是四维的
    
    CP0_input=x.reshape((batch_size,1,MR.IMAGE_SIZE,MR.IMAGE_SIZE))
    '''
    ...
    故layer0输出为（batch_size,nkerns[0],50,50)
    '''
    ...
    故layer1输出为(batch_size,nkerns[1],34,34)
    '''
    ...
    因为每个batch有batch_size，第三层有nkerns[2]个卷积核
    故layer1输出为(batch_size,nkerns[2],10,10)
    '''
    ...
    CP2=LeNetConvPoolLayer(
        ...
        input=CP1.output,
        #输入nkerns[0]张特征图，即layer0输出
        image_shape=(batch_size,nkerns[1],image_size,image_size),
    ...
    将layer2的输出(batch_size,nerkns[2],5,5)flatten为
    (batch_size,nkerns[2]*6*6)=(120,1800)
    表示500个样本，每一行代表一个样本。layer3输出大小(batch_size,n_out)=(500.500)
    '''
...
#========================================================20161024
    ...
    train_model = theano.function(  
        ...
        givens={  
            ...
            hist:train_set_x2[index * batch_size: (index + 1) * batch_size],
    ...
    validate_model = theano.function(  
        ...
        outputs=LR.errors(y),  
        givens={  
            hist:valid_set_x2[index * batch_size: (index + 1) * batch_size],
    ...
    analyse_model=theano.function(  
        on_unused_input='ignore',
        inputs=[],  
        outputs=[LR.errors(y), cost,LR.getConfusion_matrix(y)],
        givens={  
            hist:valid_set_x2,
            x: valid_set_x,  
            y: valid_set_y,                                    
    ...
    while(epoch<n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            #cPickle.dump(stroage_best_params, write_file, -1)    
            #write_file.close()  
    ...
    train_params=[
        n_epochs,learning_rate,batch_size,nkerns,sizekerns
    ]
    errors,cost,confusion_matrix=analyse_model()
    rd_iter=rd_iter[1:rd_index]
    rd_error=rd_error[1:rd_index]*100.0
    rd_error2=rd_error2[1:rd_index]*100.0
    tend=[rd_index,rd_iter,rd_error,rd_error2]
    print rd_index
    output=[train_params,errors,cost,confusion_matrix,tend]
    outfilename=str(train_params)+'_result.pickle'
    mywrite(output,outfilename)
    ...
    for apply_set_name in apply_set_namelist:
        ...
        while EOF==False:  
            ...
            if n_apply_batches==0:
                break
    #        if n_apply_batches<batch_size:
    #            batch_size=n_apply_batches#适应数据比批处理数量小
            n_apply_batches /= batch_size
            ...
            apply_model=theano.function(
                ...
                outputs=LR.getPred(),
                givens={
                    hist:apply_data2[index * batch_size: (index + 1) * batch_size],
                    x:   apply_data [index * batch_size: (index + 1) * batch_size], 
                    is_train:np.cast['int32'](0)
                    }
            )
            print 'apply .....'    
            ...
            for minibatch_index in xrange(n_apply_batches):
                ...
                prediction[minibatch_index*batch_size:(minibatch_index+1)*batch_size]=pred
                #print '%d / %d \r'%((minibatch_index+1)*batch_size,n_apply_batches),
...
def LENET():
    ...
    skip_training=False#####测试用Ture，记得改回False！！！！！！！！！！
    
    #加载数据
    Num=10
    exist_params,params=load_params('params.py')
    #skip_training=False #####测试用True，记得改回False！！！！！！！！！！
    train_name='train_'+str(MR.IMAGE_SIZE)+'.csv'
#    if exist_params :           
#        re=raw_input('Would you skip train(y/n)')
#        if re in {'Y','y'} :
#            skip_training=True 
    ...
    for i in range(Num):#Num
        ...
        evaluate_lenet(datasets,params,False,[],n_epochs=100,learning_rate=0.000005)
        
        exist_params,params=load_params('params.py')  
    evaluate_lenet(datasets,params,True,['0','1','2'],n_epochs=0,batch_size=1)
...
if __name__=='__main__':
    batch_size_list=[40,50,60,70,80]
    filter_size_list=[
        [5,4,3],[7,5,4],[9,6,5],[9,8,6],[11,7,4]
    ]
    contents=[batch_size_list,filter_size_list]
    myiter=myIteration(contents)

    while (myiter.isEmpty()==False):
        v=myiter.getvalue()
        result=[100,99]
        tend=[10,np.array(range(10)),np.ones(10)]
        '''
        df=pd.DataFrame()
        df['params']=[v]
        df['result']=[result]
        ...
        '''
        np_save=np.array([v,result,tend])
        mywrite(np_save,"test.pickle")
        #print(v)
        myiter.inc()
        
    #readf=pd.read_csv('test.csv')
    #params=readf.iloc[:,0][0]
